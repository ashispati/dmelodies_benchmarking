...
class InterpVAETrainer(DMelodiesVAETrainer):
    def __init__(
            self,
            dataset,
            model: InterpVAE,
            model_type='interp-VAE',
            lr=1e-4,
        ...
        super(InterpVAETrainer, self).__init__(
    ...
    def loss_and_acc_for_batch(self, batch, epoch_num=None, batch_num=None, train=True):
        ...
        reg_loss = self.compute_reg_loss(z_tilde, latent_attributes, self.gamma)
    ...
    def compute_reg_loss(self, z, labels, gamma):
        loss = 0
        assert len(self.attr_dict.keys()) == len(self.model.attr_class_list)
